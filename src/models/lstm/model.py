...
class StockPriceLSTM(nn.Module):
    ...
    def __init__(self,
        input_size: int,            # OHLCV + technical indicators (14 total)
        sequence_length: int,       # 60 minutes of history
        hidden_size: int,           # LSTM hidden units
        num_layers: int,            # LSTM layers
        dropout: float,             # Dropout rate
        prediction_horizon: int,    # Number of minutes to predict ahead
        **kwargs
        ):
        """
        ...
        Args:
            ...
            prediction_horizon: Number of minutes to predict ahead (e.g., 60)
        """
        ...
        self.prediction_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_size // 4, self.prediction_horizon)  # Multi-horizon price prediction
        )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        Args:
            x: Input tensor of shape (batch_size, sequence_length: 60, input_size: 14)
        ...
        # Price prediction
        price_pred = self.prediction_head(features)
        outputs[f'price'] = price_pred

        # Confidence prediction (log-variance)
        log_var = self.confidence_head(features)
        ...
        outputs[f'confidence'] = confidence
        outputs[f'variance'] = variance
